(ns samuraipersistor.refined-consumer
  (:require [integrant.core :as ig]
            ...
            [samuraipersistor.final-track-consumer :as tracks])
...
(defmethod ig/init-key :samuraipersistor/refined-consumer
  [_ {:keys [config db]}]
  (if (true? (get-in config [:kafka :refinement-tracks-enabled?]))
    {:track-consumer (tracks/start! (assoc (:kafka config) :track-stage "refined") db true)}
